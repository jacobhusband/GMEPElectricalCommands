  ...
  public class PanelCommands
  {
    ...
    public void KEEPBREAKERS()
    {
      ...
      using (var tr = db.TransactionManager.StartTransaction())
      {
        ...
        var mid1 = new Point3d(
            (line1Start.X + line1End.X) / 2,
            (line1Start.Y + line1End.Y) / 2,
            0
        );
        var mid2 = new Point3d(
            (line2Start.X + line2End.X) / 2,
            (line2Start.Y + line2End.Y) / 2,
            0
        );
      ...
      foreach (var panelDataOG in panelDataList)
      {
        ...
        if (counter % 3 == 0)
        {
          topRightCorner = new Point3d(
              originalTopRightCorner.X,
              lowestY - 1.5 - decreaseY,
              0
          );
          // Reset lowestY
    ...
    private static Dictionary<string, object> ProcessThreePolePanel(ExcelWorksheet selectedWorksheet, int row, int col)
    {
      ...
      for (int i = panelRow; i <= lastRow; i++)
      {
        ...
        {
          description =
              selectedWorksheet.Cells[i, col].Value?.ToString().ToUpper() ?? "SPACE";
        }
        string descriptionR = "";
        if (
        ...
        bool isLeftHighlighted =
            selectedWorksheet.Cells[i, col].Style.Fill.BackgroundColor.LookupColor()
            != "#FF000000";
      ...
      var panel = new Dictionary<string, object>
      {
        ...
        ["subtotal_a"] =
              selectedWorksheet.Cells[row + 2, col + 17].Value?.ToString() ?? "0",
        ["subtotal_b"] =
              selectedWorksheet.Cells[row + 2, col + 18].Value?.ToString() ?? "0",
        ["subtotal_c"] =
              selectedWorksheet.Cells[row + 2, col + 19].Value?.ToString() ?? "0",
        ["total_va"] = selectedWorksheet.Cells[row + 4, col + 17].Value?.ToString() ?? "0",
        ["lcl"] = selectedWorksheet.Cells[row + 7, col + 17].Value?.ToString() ?? "0",
        ["lcl_125"] = "0",
        ["total_other_load"] =
              selectedWorksheet.Cells[row + 10, col + 17].Value?.ToString() ?? "0",
        ["kva"] = selectedWorksheet.Cells[row + 13, col + 17].Value?.ToString() ?? "0",
        ["feeder_amps"] =
              selectedWorksheet.Cells[row + 16, col + 17].Value?.ToString() ?? "0",
    ...
    private static List<Dictionary<string, object>> ProcessWorksheet(ExcelWorksheet selectedWorksheet)
    {
      ...
      for (int row = 1; row <= rowCount; row++)
      {
        for (int col = 1; col <= colCount; col++)
        {
          ...
          string phaseAMaybe = selectedWorksheet
              .Cells[row + 3, col + 2]
              .Value?.ToString();
    ...
    private void CreateCircleText(BlockTableRecord btr, Transaction tr, Point3d centerPoint, double height, int colorIndex, string textStyle, string content)
    {
      ...
      {
        text.TextStyleId = tr.GetObject(
            textStyleTable[textStyle],
            OpenMode.ForRead
        ).ObjectId;
      }
      else
    ...
    private void CreateCalculations(BlockTableRecord btr, Transaction tr, Point3d startPoint, Point3d endPoint, Dictionary<string, object> panelData)
    {
      ...
      {
        ed.WriteMessage(
            $"Error: Unable to convert 'kva' value to double: {panelData["kva"]}"
        );
      }
        ...
        foreach (var note in customNotes)
        {
          if (!note.Contains("NOT ADDED AS NOTE"))
          {
            ...
            CreateCircle(
                ...
                new Point3d(
                    endPoint.X - 5.8088,
                    endPoint.Y - 0.3664 - (lines_of_text * 0.1872),
                    0
                ),
                0.09,
                2,
    ...
    private int ProcessTextData(Transaction tr, BlockTableRecord btr, Point3d startPoint, Dictionary<string, object> panelData, bool is2Pole)
    {
      ...
      if (panelData.ContainsKey("description_left_tags"))
      {
        ...
        List<string> descriptionRightTags =
            (List<string>)panelData["description_right_tags"];
        ...
        largest_level =
            left_largest_level > right_largest_level
                ? left_largest_level
                : right_largest_level;
      }
    ...
    private Dictionary<string, List<int>> ConvertBooleansToLevels(Dictionary<string, List<bool>> notesWithBools)
    {
      ...
      List<int> maxLevelsSoFar = Enumerable
          .Repeat(0, notesWithBools.First().Value.Count)
          .ToList();
    ...
    private void ProcessLineData(Transaction tr, BlockTableRecord btr, Point3d startPoint, Point3d endPoint, double endOfDataY, bool is2Pole)
    {
      ...
      {
                (
                    new double[] { 2.2222, 5.0666, 6.9368 },
                    startPoint.Y,
                    startPoint.Y - 0.3744,
                    layerName
                ),
