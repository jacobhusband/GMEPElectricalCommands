    ...
    public partial class UserInterface : UserControl
    {
        ...
        private void deselect_cells()
        {
            ...
            PHASE_SUM_GRID.DefaultCellStyle.SelectionForeColor = PHASE_SUM_GRID
                .DefaultCellStyle
                .ForeColor;
            TOTAL_VA_GRID.DefaultCellStyle.SelectionBackColor = TOTAL_VA_GRID
                .DefaultCellStyle
                .BackColor;
            TOTAL_VA_GRID.DefaultCellStyle.SelectionForeColor = TOTAL_VA_GRID
                .DefaultCellStyle
                .ForeColor;
        ...
        public Dictionary<string, object> retrieve_data_from_modal()
        {
            ...
            panel.Add(
                "subtotal_a",
                Math.Round(
                        Convert.ToDouble(PHASE_SUM_GRID.Rows[0].Cells[0].Value.ToString().ToUpper())
                    )
                    .ToString()
            );
            panel.Add(
                "subtotal_b",
                Math.Round(
                        Convert.ToDouble(PHASE_SUM_GRID.Rows[0].Cells[1].Value.ToString().ToUpper())
                    )
                    .ToString()
            );
            ...
            if (PHASE_SUM_GRID.Columns.Count > 2)
            {
                panel.Add(
                    ...
                    Math.Round(
                            Convert.ToDouble(
                                PHASE_SUM_GRID.Rows[0].Cells[2].Value.ToString().ToUpper()
                            )
                        )
                        .ToString()
                );
            ...
            for (int i = 0; i < PANEL_GRID.Rows.Count; i++)
            {
                ...
                if (
                    ...
                    && !string.IsNullOrEmpty(
                        PANEL_GRID.Rows[i].Cells["breaker_left"].Value as string
                    )
                )
                {
                ...
                if (
                    ...
                    && !string.IsNullOrEmpty(
                        PANEL_GRID.Rows[i].Cells["breaker_right"].Value as string
                    )
                )
                {
                    // check that the breaker value is both an integer and greater than 3
                ...
                string circuitLeftValue =
                    PANEL_GRID
                        ...
                        .ToUpper()
                        .Replace("\r", "") ?? "";
                string phaseALeftTag =
                    PANEL_GRID.Rows[i].Cells["phase_a_left"].Tag?.ToString() ?? "";
                string phaseBLeftTag =
                    PANEL_GRID.Rows[i].Cells["phase_b_left"].Tag?.ToString() ?? "";
                string phaseARightTag =
                    PANEL_GRID.Rows[i].Cells["phase_a_right"].Tag?.ToString() ?? "";
                string phaseBRightTag =
                    PANEL_GRID.Rows[i].Cells["phase_b_right"].Tag?.ToString() ?? "";
                ...
                if (PHASE_SUM_GRID.Columns.Count > 2)
                {
                    ...
                    phaseCRightTag =
                        PANEL_GRID.Rows[i].Cells["phase_c_right"].Tag?.ToString() ?? "";
                    phaseCLeftValue = (
                        PANEL_GRID
                            .Rows[i]
                            .Cells["phase_c_left"]
                ...
                {
                    var splitValues = phaseALeftValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                {
                    var splitValues = phaseBLeftValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                {
                    var splitValues = phaseARightValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                {
                    var splitValues = phaseBRightValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                if (PHASE_SUM_GRID.Columns.Count > 2)
                {
                    ...
                    {
                        var splitValues = phaseCLeftValue
                            .Split(',')
                            .Select(str => str.Trim())
                            .ToArray();
                    ...
                    {
                        var splitValues = phaseCRightValue
                            .Split(',')
                            .Select(str => str.Trim())
                            .ToArray();
                ...
                if (descriptionLeftValue.Contains(","))
                {
                    ...
                    var splitValues = descriptionLeftValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                {
                    // If it contains a comma, split and add both values
                    var splitValues = breakerLeftValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                {
                    // If it contains a comma, split and add both values
                    var splitValues = descriptionRightValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
                ...
                {
                    // If it contains a comma, split and add both values
                    var splitValues = breakerRightValue
                        .Split(',')
                        .Select(str => str.Trim())
                        .ToArray();
            ...
            for (int i = 0; i < columnIndex.Length; i += 2)
            {
                ...
                foreach (DataGridViewRow row in PANEL_GRID.Rows)
                {
                    for (int j = 0; j < 2; j++)
                    {
                        ...
                        {
                            var hasLCLApplied = verify_LCL_from_phase_cell(
                                row.Index,
                                columnIndex[i + j]
                            );
        ...
        public void calculate_lcl_otherload_panelload_feederamps()
        {
            if (LARGEST_LCL_CHECKBOX.Checked)
            {
                ...
                if (double.TryParse(largestLclInputText, out largestLclInputValue))
                {
                    ...
                    {
                        double result =
                            (panelLoad + difference) / (lineVoltage * PHASE_SUM_GRID.ColumnCount);
        ...
        public void populate_modal_with_panel_data(Dictionary<string, object> selectedPanelData)
        {
            ...
            {
                multi_row_datagrid_keys.AddRange(
                    new List<string> { "phase_c_left", "phase_c_right" }
                );
            }
            ...
            for (int i = 0; i < length * 2; i += 2)
            {
                foreach (string key in multi_row_datagrid_keys)
                {
                    ...
                    {
                        List<string> values = (
                            (Newtonsoft.Json.Linq.JArray)selectedPanelData[key]
                        ).ToObject<List<string>>();
        ...
        private void link_cell_to_phase(
            string cellValue,
            DataGridViewRow row,
            ...
            panelGrid_eventHandler = (sender, e) =>
            {
                if (e.RowIndex == row.Index && e.ColumnIndex == col.Index)
                {
                    ...
                    if (newCellValue != phaseSumGridValue)
                    {
                        ...
                        panelGrid.Rows[row.Index].Cells[col.Index].Style.BackColor =
                            Color.LightGray;
                    ...
                    if (cell.OwningColumn.Name.Contains("description"))
                    {
                        ...
                        if (
                            cell
                                .Tag.ToString()
                                .Split('|')
                                .Contains(APPLY_COMBOBOX.SelectedItem.ToString())
                        )
                        {
        ...
        private double get_equipment_load(int rowIndex, int colIndex)
        {
            ...
            )
            {
                ...
                var nextBreakerCellValue = PANEL_GRID
                    ...
                    .Cells["breaker" + side]
                    .Value;
                var breakerCellValueTwoRowsDown = PANEL_GRID
                    .Rows[rowIndex + 2]
                    .Cells["breaker" + side]
                    .Value;
                ...
                if (
                    ...
                    && breakerCellValue.ToString() != ""
                    && breakerIntValue > 4
                )
                {
                    int rowsToCheck = 1;
                    if (
                        nextBreakerCellValue == null
                        && breakerCellValueTwoRowsDown != null
                        && breakerCellValueTwoRowsDown.ToString() == "3"
                    )
                    {
                        rowsToCheck = 3;
                    }
        ...
        )
        {
            if (phaseSumGridColumnCount == panelPhaseSumGridColumnCount)
            {
                ...
                if (PANEL_GRID.Rows.Count > rowIndex + rowCount - 1)
                {
                    ...
                    var cellValueC = phaseSumGridColumnCount == 3 ? "=" + panelName + "-C" : null;
        ...
        )
        {
            if (cellValue.StartsWith("="))
            {
                ...
                {
                    var result = new System.Data.DataTable().Compute(
                        cellValue.Replace("=", ""),
                        null
                    );
                    PANEL_GRID.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Convert.ToDouble(
                        result
                    );
                }
                else
        ...
        private void auto_link_subpanels(
            string cellValue,
            DataGridViewRow row,
            DataGridViewColumn col
        )
        ...
        private void PANEL_GRID_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (
                PANEL_GRID.Rows[e.RowIndex].Cells[e.ColumnIndex].Value == null
                || PANEL_GRID.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString() == ""
            )
                return;
        ...
        private void PANEL_GRID_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            selectedCells = new List<DataGridViewCell>(
                PANEL_GRID.SelectedCells.Cast<DataGridViewCell>()
            );
            ...
            if (e.Control && e.KeyCode == Keys.V)
            {
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    foreach (string line in lines)
                    {
                        ...
                        {
                            if (
                                rowIndex < PANEL_GRID.RowCount
                                && colIndex + i < PANEL_GRID.ColumnCount
                            )
                            {
                                try
                                {
        ...
        private async void PANEL_GRID_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (cell.Tag != null)
            {
                ...
                if (
                    INFO_LABEL.Text
                    == $"This cell is linked to phase {phase} of panel '{panelName}'."
                )
                {
        ...
        private void PANEL_GRID_EditingControlShowing(
            object sender,
        ...
        private void PANEL_GRID_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0) // Check if it's not the header cell
            {
                ...
                if (cell.Selected)
                {
                    e.Paint(
                        e.CellBounds,
                        DataGridViewPaintParts.All & ~DataGridViewPaintParts.Border
                    );
